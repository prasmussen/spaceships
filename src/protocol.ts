...
function validateFields(p:InputPacket){
  if(!Number.isInteger(p.epoch)||p.epoch<0||p.epoch>0xffffffff||![0,1].includes(p.sender)||!Number.isInteger(p.start)||p.start<0||p.start>2147483400||!Number.isInteger(p.ack)||p.ack<1||p.ack>2147483527||!Array.isArray(p.frames)||p.frames.length<1||p.frames.length>8||p.frames.some(b=>!Number.isInteger(b)||b<0||b>31))throw new ProtocolError('Malformed input packet');
}
