...
function validateFields(p:InputPacket){
  if(!Number.isInteger(p.epoch)||p.epoch<0||p.epoch>0xffffffff||![0,1].includes(p.sender)||!Number.isInteger(p.start)||p.start<0||p.start>2147483400||!Number.isInteger(p.ack)||p.ack<1||p.ack>2147483527||!Array.isArray(p.frames)||p.frames.length<1||p.frames.length>8||p.frames.some(b=>!Number.isInteger(b)||b<0||b>15))throw new ProtocolError('Malformed input packet');
}
...
export type Control =
 | {type:'hello';matchId:string;epoch:number;slot:number;identity:{protocol:number;abi:number;wasm:string;map:string;config:string};inputDelay:number}
 | {type:'ready'} | {type:'start';delayMs:number;oneWayMs:number} | {type:'started'}
...
export function decodeControl(raw:string):Control {
  ...
  let valid=false;
  switch(m.type){
    case 'hello':{const i=m.identity as Record<string,unknown>;valid=typeof m.matchId==='string'&&/^[a-f0-9]{32}$/.test(m.matchId)&&integer(m.epoch,0,0xffffffff)&&integer(m.slot,0,1)&&m.inputDelay===2&&!!i&&i.protocol===PROTOCOL&&integer(i.abi,1,100)&&digest(i.wasm)&&digest(i.map)&&digest(i.config);break;}
    case 'ready':case 'started':valid=true;break;
    ...
    case 'repair':valid=integer(m.start)&&integer(m.ack,1)&&Array.isArray(m.frames)&&m.frames.length>0&&m.frames.length<=120&&m.frames.every(b=>integer(b,0,15));break;
    case 'need':valid=integer(m.from)&&integer(m.to)&&(m.to as number)>=(m.from as number)&&(m.to as number)-(m.from as number)<120;break;
    case 'hash':case 'recovered':valid=integer(m.tick)&&hash(m.hash);break;
    case 'recover':valid=integer(m.tick)&&Array.isArray(m.snapshot)&&m.snapshot.length===8384&&m.snapshot.every(b=>integer(b,0,255));break;
