...
function hud(player:number){const o=16+player*16;return `<span>FUEL <strong>${Math.ceil(state[o+6]/60)}%</strong></span><span>SPEED <strong>${(Math.hypot(state[o+2],state[o+3])/65536*60).toFixed(0)}</strong></span><span>BOOST <strong>${state[o+7]===0?'—':state[o+13]>0?'ACTIVE':state[o+14]>0?`${(state[o+14]/60).toFixed(1)}s`:state[o+6]<=120?'LOW FUEL':'READY'}</strong></span><span>SPIN <strong>${state[o+5]}</strong></span><span>HULL <strong>${state[o+7]} / 3</strong></span><span>SCORE <strong>${state[o+11]}</strong></span><small>${state[o+7]===0?'RESPAWNING':state[o+8]?'ON PAD · REFUELING':'IN FLIGHT'} · FIRST TO 5</small>`;}
...
worker.onmessage=({data})=>{
  if(data.type==='frame'){
    ...
    practiceInputs=data.inputs;state=new Int32Array(data.buffer);effects.consume(state);status.innerHTML=hud(0)+scores()+(mode===0?'<small>COMPUTER · PATROL & BURST FIRE</small>':'');
...
online=new OnlineLobby({
  ...
  frame:(data,slot)=>{if(mode!==3)return;onlineButtons=data.inputs;correction=data.rollbacks+data.desyncs;state=new Int32Array(data.buffer);effects.consume(state);status.innerHTML=hud(slot)+scores();result.hidden=state[1]<0;if(state[1]>=0)document.querySelector('#winner')!.textContent=`Player ${state[1]+1} wins · ${Array.from({length:state[5]},(_,id)=>state[27+id*16]).join(' : ')} · unverified`;},
