...
function hud(player:number){const o=16+player*16;return `<div class="hud-vitals"><span>HULL <strong>${Math.ceil(state[o+7]/6)}%</strong></span><span>ENERGY <strong>${Math.ceil(state[o+6]/60)}%</strong></span></div><div class="hud-abilities"><span>BOOST <strong>${state[o+7]===0?'—':state[o+13]>0?'ACTIVE':state[o+14]>0?`${(state[o+14]/60).toFixed(1)}s`:state[o+6]<=120?'LOW ENERGY':'READY'}</strong></span><span>SHIELD <strong>${state[o+7]===0?'—':(state[6+player*2]&255)>0?'ACTIVE':(state[6+player*2]>>>16)>0?`${((state[6+player*2]>>>16)/60).toFixed(1)}s`:state[o+6]<1500?'LOW ENERGY':'READY'}</strong></span></div><div class="hud-flight"><span>SPEED <strong>${(Math.hypot(state[o+2],state[o+3])/65536*60).toFixed(0)}</strong></span><span>SPIN <strong>${state[o+5]}</strong></span><span>SCORE <strong>${state[o+11]}</strong></span></div><small>${state[o+7]===0?'RESPAWNING':state[o+8]?'ON PAD · RECHARGING / REPAIRING':'IN FLIGHT'}</small>`;}
