import {STATE_BYTES, type Engine} from './engine.ts';
export type Inputs=readonly number[];
export class ProtocolError extends Error {}
/** Tick means next tick to simulate. Frontiers are inclusive input ticks. */
export class Rollback {
  tick=0;
  complete=1;
  private acknowledgements:number[];
  get peerAck(){return Math.min(...this.acknowledgements.filter((_,id)=>id!==this.player));}
  rollbacks=0;
  maxDepth=0;
  stalls=0;
  frozen=false;
  readonly input:Map<number,number>[];
  readonly executed=new Map<number,Inputs>();
  ...
  receive(player:number,tick:number,buttons:number){
    if(!Number.isInteger(player)||player<0||player>=this.engine.players||player===this.player || !Number.isInteger(tick) || tick<0 || tick>this.tick+120 || !Number.isInteger(buttons)||buttons<0||buttons>63)throw new ProtocolError('Invalid input ownership, tick window or buttons');
  ...
  acknowledge(tick:number,player:number){
    if(!Number.isInteger(player)||player<0||player>=this.engine.players||player===this.player||!Number.isInteger(tick)||tick<1||tick>this.tick+120)throw new ProtocolError('Invalid acknowledgement');
    this.acknowledgements[player]=Math.max(this.acknowledgements[player],tick);
  }
  private updateComplete(){while(this.input.every(input=>input.has(this.complete+1)))this.complete++;}
  /** Called at fixed scheduling cadence. A stalled tick never creates extra commands. */
  advance(buttons:number){
    if(!Number.isInteger(buttons)||buttons<0||buttons>63)throw new ProtocolError('Invalid local buttons');
  ...
  private execute(){
    const previous=this.executed.get(this.tick-1)??Array(this.engine.players).fill(0);
    const pair:Inputs=this.input.map((input,id)=>input.get(this.tick)??previous[id]);
  ...
  confirmedInputs(){const pairs:Inputs[]=[];for(let t=0;t<=Math.min(this.agreed,this.tick-1);t++){pairs.push(this.input.map(input=>{const buttons=input.get(t);if(buttons===undefined)throw Error('Incomplete replay');return buttons;}));}return pairs;}
}
