...
export function checkReplay(value:unknown,identity:Identity):asserts value is Replay {
  ...
  if(!snapshot(r.initial)||!r.inputs.every(pair=>Array.isArray(pair)&&pair.length===2&&pair.every(b=>Number.isInteger(b)&&b>=0&&b<=31)))throw Error('Invalid replay state or inputs');
