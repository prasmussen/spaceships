...
import type {Inputs} from './rollback.ts';
export interface Identity {protocol:number;abi:number;wasm:string;map:string;config:string}
export interface Replay {version:2;players:number;identity:Identity;seed:number;mapMode:number;initial:number[];inputs:Inputs[];checkpoints:{tick:number;hash:string;state:number[]}[]}
...
export async function recordReplay(binary:BufferSource,identity:Identity,inputs:Inputs[],mapMode=32768,seed=0,players=2):Promise<Replay>{
  const engine=await Engine.create(binary,mapMode,seed,players);
  const replay:Replay={version:2,players,identity,seed,mapMode,initial:[...engine.save()],inputs,checkpoints:[]};
...
export class ReplayPlayer {
  ...
  tick=0;
  constructor(engine:Engine,replay:Replay){if(engine.players!==replay.players||engine.map!==replay.mapMode||engine.seed!==replay.seed)throw Error('Replay engine configuration mismatch');this.engine=engine;this.replay=replay;this.engine.load(new Uint8Array(replay.initial));}
  seek(tick:number){
  ...
  async validate(){
    ...
    engine.wasm.init(1024,replay.mapMode,replay.seed,replay.players);
