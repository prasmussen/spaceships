...
export class EventDeduper {
  ...
  consume(frame:Int32Array){
    ...
    const result:GameEvent[]=[],count=frame[2096]??0;
    if(count<0||2097+count*8>frame.length)throw Error('Invalid event frame');
    for(let i=0;i<count;i++){
      const o=2097+i*8,[tick,entity,index,type,x,y,player,data]=frame.subarray(o,o+8);
