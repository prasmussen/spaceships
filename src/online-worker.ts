...
onmessage=({data})=>{chain=chain.then(async()=>{
  if(data.type==='init'){
    ...
    session=new NetworkSession(await Engine.create(binary,32768,data.match.seed),data.slot,data.match,{
      gameplay:buffer=>postMessage({type:'gameplay',buffer},[buffer]),control:message=>send('control',{message}),diagnostic:bundle=>send('diagnostic',{bundle})
    });send('booted');
  ...
  else if(data.type==='gameplay')session?.gameplay(data.buffer);
  else if(data.type==='control')session?.control(data.raw);
