...
export async function checkDeployment(browser){
  ...
  const values={SITE_ADDRESS:'localhost',PUBLIC_ORIGIN:'https://localhost:18443',BIND_ADDRESS:'127.0.0.1',HTTP_PORT:'18080',HTTPS_PORT:'18443',TLS_CONFIG:'tls internal',METRICS_TOKEN:randomBytes(32).toString('hex'),REGIONS:'eu',TURN_URLS:'',TURN_SECRET:'',APP_MEMORY_LIMIT:'512m',APP_CPUS:'2.0',APP_GO_MEMORY_LIMIT:'384MiB'};
  ...
  try{
    ...
    assert.equal(await pages[0].evaluate(()=>{const script=document.createElement('script');script.textContent='window.inlineScriptRan = true';document.head.append(script);script.remove();return !!window.inlineScriptRan;}),false);
    ...
    await pages[0].getByRole('button',{name:'Open game menu'}).click();await pages[0].getByRole('button',{name:'Exit to lobby',exact:true}).click();
    await pages[1].waitForFunction(()=>/Left match|disconnected/i.test(document.querySelector('#connection-status').textContent));
    const {stdout:appID}=await compose(['ps','-q','app']);
    const {stdout:limits}=await exec('docker',['inspect','--format','{{.HostConfig.Memory}} {{.HostConfig.NanoCpus}} {{.HostConfig.PidsLimit}}',appID.trim()]);
    assert.deepEqual(limits.trim().split(' ').map(Number),[512*1024*1024,2e9,128]);
    const {stdout:memory}=await exec('docker',['stats','--no-stream','--format','{{.MemUsage}}',appID.trim()]);
    await writeFile('artifacts/https-browser.json',JSON.stringify({https:true,csp:true,hsts:true,containerLimits:limits.trim(),sampledMemory:memory.trim(),secureCookies:true,wssQueue:true,webRTC:true,originRejection:true,wasm:manifest.wasm,leave:true},null,2)+'\n');
