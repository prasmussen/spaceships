...
function ship(st,id,x,y){const o=16+16*id;st.set([x*Q,y*Q,0,0,0,0,6000,3,0,0,0,0,0,0,0,0],o);}
...
test('all four players may land on every shared pad',async()=>{
  for(let id=0;id<4;id++)for(const pad of cave.pads){const {engine,st}=await game(4,32769);
    ...
    assert.equal(st[o+8],1);assert.equal(st[o+7],3);engine.load(engine.save());
  }
});
test('respawn maximizes minimum distance across all three living opponents',async()=>{
  for(let victim=0;victim<4;victim++){
    ...
    others.forEach((id,i)=>ship(st,id,...[[900,1300],[5300,3400],[4500,1200]][i]));
    ...
    engine.step([0,0,0,0]);assert.equal(st[o],best.x*Q);assert.equal(st[o+1],(best.y-16)*Q-1);assert.equal(st[o+7],3);
...
test('projectiles hit the nearest opponent and credit the lethal shooter in slots 2 and 3',async()=>{
  for(const owner of [2,3]){const {engine,st,pool}=await game();
    ...
    ship(st,0,100,0);ship(st,1,50,0);st[39]=1;
    pool.set([20*Q,0,76*Q,0,10,owner,1,0]);st[3]=1;
    engine.step([0,0,0,0]);assert.equal(st[39],0);assert.equal(st[23],3);assert.equal(st[27],0);assert.equal(st[16+owner*16+11],1);
    engine.load(engine.save());
  }
});
test('every hull pair collides, including slots 2 and 3',async()=>{
  for(let a=0;a<4;a++)for(let b=a+1;b<4;b++){
    ...
    engine.step([0,0,0,0]);for(let id=0;id<4;id++){assert.equal(st[23+id*16],id===a||id===b?0:3);assert.equal(st[27+id*16],id===a||id===b?-1:0);}
  }
});
test('four-way simultaneous kills preserve trades and top-score ties, then a unique leader wins',async()=>{
  ...
  for(let id=0;id<4;id++){ship(st,id,id*300,0);st[23+id*16]=1;st[27+id*16]=4;pool.set([id*300*Q,0,0,0,10,(id+1)%4,id+1,0],id*8);}st[3]=4;
  ...
  ship(st,0,0,0);st[23]=1;st[27]=5;pool.set([0,0,0,0,10,3,st[3]+1,0]);st[3]++;
...
test('higher-slot exhaust and debris affect opponents without hull damage',async()=>{
  ...
  ship(st,2,0,0);ship(st,3,0,50);engine.step([0,0,1,0]);assert.ok(st[67]>3600);assert.equal(st[71],3);
  ship(st,2,0,0);ship(st,3,50,0);st[66]=-Q;pool.set([25*Q,0,40*Q,0,186,2,1,0x10000000]);st[3]=1;
  engine.step([0,0,0,0]);assert.equal(st[71],3);assert.ok(pool[2]<0,'fragment bounces from slot 3');
});
