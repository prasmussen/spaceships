...
test('a complete first-to-five match freezes at the winning tick',async()=>{const {s,st}=await create();for(let kill=0;kill<5;kill++){arena(st);step(s,8,0,24);assert.equal(st[43],0);assert.equal(st[27],kill+1);}assert.equal(st[1],0);const hash=s.state_hash();step(s,15,15,60);assert.equal(s.state_hash(),hash);s.save_state(65536);assert.equal(s.load_state(65536,8384),1);});
...
test('spawn protection expires, and ends immediately upon firing or leaving pad',async()=>{const {s,st}=await create(32768);assert.equal(st[28],90);step(s,0,0,89);assert.equal(st[28],1);step(s);assert.equal(st[28],0);s.init(1024,32768,0);step(s,8);assert.equal(st[28],0);s.init(1024,32768,0);step(s,1);assert.equal(st[28],0);});
test('protected target takes no projectile damage',async()=>{const {s,st,pool}=await create(32768);pool.set([450*Q,1684*Q,0,0,10,1,1,0]);st[3]=1;step(s);assert.equal(st[23],3);assert.equal(pool[4],9);});
test('pool allocation chooses the lowest free slot and snapshots include reuse state',async()=>{const {s,st,pool}=await create();arena(st);for(let i=0;i<256;i++)pool.set([0,-500*Q,0,0,100,0,i+1,0],i*8);st[3]=256;step(s,8);assert.equal(st[3],256);pool.fill(0,7*8,8*8);step(s,8);assert.equal(pool[7*8+6],257);s.save_state(65536);step(s,8,8,30);const hash=s.state_hash();assert.equal(s.load_state(65536,8384),1);step(s,8,8,30);assert.equal(s.state_hash(),hash);});
test('malformed projectile snapshots are rejected without partial restoration',async()=>{const {s}=await create();step(s,8);for(const [field,value]of [[0,-2147483648],[2,4980737],[4,121],[5,2],[6,0],[7,1]]){s.save_state(65536);new DataView(s.memory.buffer).setInt32(65536+192+field*4,value,true);const hash=s.state_hash();assert.equal(s.load_state(65536,8384),0);assert.equal(s.state_hash(),hash);}});
test('moving-target sweep detects a hit even when both endpoints miss',async()=>{const {s,st,pool}=await create();st[32]=0;st[33]=0;st[34]=64*Q;pool.set([50*Q,0,-76*Q,0,10,0,1,0]);st[3]=1;step(s);assert.equal(st[39],2);assert.equal(pool[4],0);});
test('terrain absorbs projectiles before a ship beyond the pillar',async()=>{const {s,st,pool}=await create(32768);for(let p=0;p<2;p++){const o=16+p*16;st[o]=(p?1800:1400)*Q;st[o+1]=1000*Q;st[o+4]=p?3072:1024;st[o+8]=0;st[o+12]=0;}step(s,8);step(s,0,0,9);assert.equal(st[39],3);assert.equal(st[27],0);assert.ok([...pool].filter((_,i)=>i%8===4).every(v=>v===0));});
test('snapshot validation rejects duplicate projectile IDs and unsafe remaining travel',async()=>{const {s}=await create();step(s,8,8);s.save_state(65536);let view=new DataView(s.memory.buffer);view.setInt32(65536+192+32+24,1,true);assert.equal(s.load_state(65536,8384),0);s.save_state(65536);view.setInt32(65536+192,1670000000,true);view.setInt32(65536+192+8,4980736,true);assert.equal(s.load_state(65536,8384),0);});
for(const target of [0,1])test(`same-tick crash and lethal projectile resolve consistently for player ${target}`,async()=>{const {s,st,pool}=await create(32768);const o=16+target*16,other=16+(1-target)*16;st[o]=1420*Q;st[o+1]=1000*Q;st[o+2]=64*Q;st[o+7]=1;st[o+8]=0;st[o+12]=0;st[other]=1800*Q;st[other+1]=1000*Q;st[other+8]=0;st[other+12]=0;pool.set([1400*Q,1000*Q,76*Q,0,10,1-target,1,0]);st[3]=1;step(s);assert.equal(st[o+7],0);assert.equal(st[o+11],-1);assert.equal(st[other+11],0);});
