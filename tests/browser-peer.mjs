  ...
  try{
    for(let p=0;p<2;p++){
      ...
      await page.exposeFunction('routeSignal',async signal=>{
        const remote=1-p;if(pending[remote])pending[remote].push(signal);
        else await pages[remote].evaluate(signal=>window.netPeer.signal(signal),signal);
      });
    }
    for(let p=0;p<2;p++){
      await pages[p].evaluate(async ({slot,iceServers,relay})=>{
        ...
        window.netPeer=new PeerMatch({id:'1234567890abcdef1234567890abcdef',epoch:4100,seed:19,identity,inputDelay:2,recoveryPeer:0,region:'eu'},slot,iceServers,{
      ...
      const saved=pending[p];pending[p]=null;for(const signal of saved)await pages[p].evaluate(signal=>window.netPeer.signal(signal),signal);
    }
    ...
    await pages[0].evaluate(()=>{window.netPeer.renewalAt=0;});
    ...
    const results=await Promise.all(pages.map(page=>page.evaluate(()=>({state:window.netLatest,ended:window.netEnded,hashes:window.netHashes,connection:window.netPeer.pc.connectionState,channels:{gameplay:{ordered:window.netPeer.gameplayChannel.ordered,retries:window.netPeer.gameplayChannel.maxRetransmits},control:{ordered:window.netPeer.controlChannel.ordered,retries:window.netPeer.controlChannel.maxRetransmits}}}))));
    ...
    for(let p=0;p<2;p++){
      await pages[p].evaluate(async slot=>{
        ...
        window.netPeer=new PeerMatch({id:'2234567890abcdef1234567890abcdef',epoch:4101,seed:19,identity,inputDelay:2,recoveryPeer:0,region:'eu'},slot,[],{
