...
const Q=65536;
async function create(){const {instance}=await WebAssembly.instantiate(binary);const s=instance.exports;assert.equal(s.init(1024,32768,0),1);return {s,st:new Int32Array(s.memory.buffer,4096,48)};}
...
test('opponent pad crashes, respawns at own base exactly two seconds later',async()=>{const {s,st}=await create();approach(st);st[16]=2750*Q;step(s);assert.equal(st[23],0);assert.equal(st[26],120);assert.equal(st[27],-1);step(s,0,119);assert.equal(st[23],0);step(s);assert.equal(st[23],3);assert.equal(st[16],450*Q);assert.equal(st[24],1);assert.equal(st[27],-1);});
