...
function approach(st){st[16]=338*Q;st[17]=1259*Q-1;st[18]=0;st[19]=30000;st[20]=0;st[21]=0;st[22]=3000;st[24]=0;}
test('grounded ships refuel, suppress rotation, and launch with thrust',async()=>{const {s,st}=await create();assert.equal(st[24],1);st[22]=3000;const y=st[17];step(s,4,10);assert.equal(st[21],0);assert.equal(st[17],y);assert.equal(st[22],3260);step(s,1);assert.equal(st[24],0);assert.ok(st[17]<y);assert.equal(st[22],3259);});
for(const [name,index,limit]of [['horizontal speed',18,tuning.landingMaxVx],['vertical speed',19,tuning.landingMaxVy],['orientation',20,tuning.landingMaxAngle],['spin',21,tuning.landingMaxSpin]]) {
  for(const delta of [-1,0,1])test(`landing ${name} at limit ${delta>=0?'+':''}${delta}`,async()=>{
    ...
    assert.equal(st[23],delta<=0?600:0);assert.equal(st[24],delta<=0?1:0);assert.equal(st[27],delta<=0?0:-1);
  });
}
for(const delta of [-1,0,1])test(`landing footprint boundary ${delta}`,async()=>{const {s,st}=await create();approach(st);st[16]=(338+75-16)*Q+delta;step(s);assert.equal(st[23],delta<=0?600:0);});
test('either player can land on every pad and restore the grounded snapshot',async()=>{
  ...
  for(const player of [0,1])for(const pad of cave.pads){
    ...
    step(s);assert.equal(st[o+7],600);assert.equal(st[o+8],1);
...
test('respawn after two seconds selects the pad farthest from the living opponent',async()=>{
  ...
  assert.equal(st[23],600);assert.equal(st[16],4462*Q);assert.equal(st[17],2759*Q-1);assert.equal(st[24],1);
});
...
test('maximum-speed flight hits the pillar before entering solid terrain',async()=>{const {s,st}=await create();approach(st);st[16]=1058*Q;st[17]=1000*Q;st[18]=64*Q;step(s);assert.equal(st[23],0);assert.ok(st[16]<=1072*Q);assert.equal(st[27],-1);});
...
for(const [name,index,limit]of [['leftward speed',18,tuning.landingMaxVx],['counterclockwise spin',21,tuning.landingMaxSpin],['wrapped orientation',20,tuning.landingMaxAngle]]) {
  for(const delta of [-1,0,1])test(`landing ${name} magnitude limit ${delta}`,async()=>{
    const {s,st}=await create();approach(st);st[index]=index===20?4096-(limit+delta):-(limit+delta+(index===21?8:0));if(index===18)st[index]+=Math.trunc(st[index]/511);step(s);assert.equal(st[23],delta<=0?600:0);
  });
}
test('contact from below cannot land',async()=>{const {s,st}=await create();approach(st);st[17]=1285*Q;st[19]=-30000;step(s);assert.equal(st[23],0);assert.equal(st[24],0);});
test('zero fuel in flight disables thrust but permits counter-steering',async()=>{const {s,st}=await create();approach(st);st[17]=1000*Q;st[19]=0;st[22]=0;step(s,5);assert.equal(st[19],3597);assert.equal(st[21],4);assert.equal(st[22],0);});
test('empty grounded tank refuels while grounded rotation stays suppressed',async()=>{const {s,st}=await create();st[22]=0;step(s,4);assert.equal(st[22],26);assert.equal(st[21],0);assert.equal(st[24],1);});
