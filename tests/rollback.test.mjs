...
const buttons=(p,t)=>((t*7+(t>>(p?3:5)))&63);
...
test('outage stalls both command generation and simulation; resume repairs prediction',async()=>{const peer=new Rollback(await Engine.create(wasm),0);for(let t=0;t<100;t++)peer.advance(t&63);assert.equal(peer.tick,14);const count=peer.input[0].size;for(let t=0;t<100;t++)peer.advance(0);assert.equal(peer.input[0].size,count);for(let t=2;t<16;t++)peer.receive(1,t,1);peer.acknowledge(15,1);assert.ok(peer.advance(0));});
for(const players of [3,4])test(`${players} peers converge with loss, reordering, duplicate input and a disconnected peer`,async()=>{
  ...
  for(let wall=0;wall<2500;wall++){
    ...
    for(const peer of peers)if(peer.tick<target)peer.advance((wall*7+peer.player*11)&63);
