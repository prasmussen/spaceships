...
async function create(){ const {instance}=await WebAssembly.instantiate(binary);const s=instance.exports;assert.equal(s.init(1024,0,0,2),1);return s; }
...
test('normal thrust uses half the energy and still runs out without disabling rotation',async()=>{const s=await create();step(s,1,0,3000);assert.equal(state(s)[22],3000);step(s,1,0,3000);assert.equal(state(s)[22],0);const vy=state(s)[19];step(s,5);assert.ok(state(s)[19]>vy);assert.ok(state(s)[21]>0);});
test('snapshot restoration reproduces exact future hashes',async()=>{const s=await create();step(s,5,2,80);const hash=s.state_hash();assert.equal(s.save_state(65536),8512);step(s,3,1,100);const future=s.state_hash();assert.equal(s.load_state(65536,8512),1);assert.equal(s.state_hash(),hash);step(s,3,1,100);assert.equal(s.state_hash(),future);});
...
test('fixed memory and invalid snapshot boundaries',async()=>{const s=await create();assert.throws(()=>s.memory.grow(1));assert.equal(s.step(0,1),0);assert.equal(s.step(2048,121),0);assert.equal(s.load_state(65536,8383),0);s.save_state(65536);new DataView(s.memory.buffer).setInt32(65536+64+16,4096,true);const hash=s.state_hash();assert.equal(s.load_state(65536,8512),0);assert.equal(s.state_hash(),hash);});
test('configuration must match the versioned artifact',async()=>{const s=await create();new DataView(s.memory.buffer).setInt32(1024+12,999,true);const hash=s.state_hash();assert.equal(s.init(1024,0,0,2),0);assert.equal(s.state_hash(),hash);});
test('snapshot rejects every out-of-bounds field and reserved region atomically',async()=>{const s=await create();const invalid=[[0,-1],[4,0],[8,1],[60,1],[64,2147483647],[68,-2147483648],[72,4194305],[76,-4194305],[80,-1],[84,73],[88,-1],[92,601],[96,1],[116,19],[120,301],[124,2],[128,-2147483648],[148,-73],[188,2]];for(const [offset,value]of invalid){s.save_state(65536);new DataView(s.memory.buffer).setInt32(65536+offset,value,true);const hash=s.state_hash();assert.equal(s.load_state(65536,8512),0,`offset ${offset}`);assert.equal(s.state_hash(),hash);}});
...
test('the final energy unit powers both normal flight substeps without going negative',async()=>{const s=await create(),st=state(s);st[22]=1;step(s,1);assert.equal(st[22],0);assert.equal(st[19],-7200);step(s,1);assert.equal(st[22],0);assert.ok(st[19]>-7200);});
