...
After admitting a page to a multiplayer match, the Go service issues ten-minute HMAC credentials compatible with coturn's REST authentication. The browser obtains them through `POST /api/ice` with its page instance and current match ID; pre-match `/api/config` contains STUN only. Issuance requests are rate-limited and reuse the guest credential until its renewal window. An explicit departure stops renewal immediately; a broken WebSocket has a ten-second grace period. Previously issued credentials remain valid until their expiry, so admission controls complement rather than replace relay quotas. The shared `TURN_SECRET` belongs on the service and relay host; browsers receive only expiring usernames and credentials.

## Local browser check

```sh
node tests/browser.mjs --server-only --turn
```
...
For an extended check, run `node tests/browser.mjs --server-only --turn --soak`. It holds real relay traffic for eleven minutes, verifies scheduled renewals beyond the original credential expiry, then checks recovery/disconnect and shared hashes. The completed result is saved in `artifacts/turn-soak.json`.
...
Monitor coturn allocation failures and bandwidth separately from `/api/metrics` server admission/relay-response counters. The default bandwidth capacity is 26,214,400 bytes/second per direction; choose a lower budget if that sustained usage is unacceptable for the host or provider bill. Match admission is not identity verification: coordinated bots can still consume their allowed quotas.
