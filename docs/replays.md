...
Keep each recording with its build manifest and WASM. The validator checks the binary SHA-256 and all identity fields, initializes the simulation from the recorded match configuration, compares the initial bytes, then executes each paired input. Every supplied checkpoint must match both the signed 64-bit hash and all 8384 snapshot bytes. Successful output contains ticks, final hash, winner (`-1` when unfinished), and both scores. Invalid files exit nonzero.

Bounds match the browser format: 216,000 ticks, 3,601 checkpoints, two button masks per input, and fixed snapshot size. The CLI additionally caps JSON at 110 MiB, execution at 30 seconds and WASM memory at four pages. No host functions or filesystem/network imports are provided to WASM. Context cancellation terminates execution through [wazero's runtime configuration](https://github.com/tetratelabs/wazero/blob/v1.9.0/config.go).

`go test -race ./...` generates a 3,600-tick recording with the JavaScript WASM engine and validates every checkpoint in wazero. It also rejects altered identities, artifacts, initial states, input shapes, checkpoint order, hashes and bytes. This requires Node 22.18+ and the built public artifacts.
